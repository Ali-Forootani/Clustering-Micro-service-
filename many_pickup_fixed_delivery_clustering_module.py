...
def many_pickup_fixed_delivery_clustering_function(time_clus,geo_clus,veh_clus,pic_del_time,pic_lon_lat):
            ...
            clustering_loc= KMeans(n_clusters=int(geo_clus), random_state=0).fit(pic_lon_lat)
            ...
            #colors_1=(0,1,0)
            #plt.figure(2, figsize=(8, 4), facecolor='w', edgecolor='k')
            #plt.scatter(pic_lon_lat[:,0], pic_lon_lat[:,1], c=colors_1, alpha=0.5)
            #plt.title('2-D random geolocations')
            #plt.xlabel('Geolocation many pickup-fixed delivery')
            #plt.ylabel('Geolocation many pickup-fixed delivery')
